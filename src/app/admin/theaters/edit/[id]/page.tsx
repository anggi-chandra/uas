...
export default function EditTheaterPage() {
  ...
  const [theaterLoaded, setTheaterLoaded] = useState(false);
  ...
  useEffect(() => {
    ...
    const checkAdminAndLoadTheater = async () => {
      try {
        setLoading(true);
        ...
        // Load theater data
        const { data: theaterData, error: theaterError } = await supabaseAdmin
          .from('theaters')
          .select('*')
          .eq('id', theaterId)
          .single();
        ...
        if (theaterData) {
          ...
          setImageUrl(theaterData.image || '');
          setFacilities(theaterData.facilities || []);
          setTheaterLoaded(true);
        }
      } catch (err) {
        console.error('Error loading theater data:', err);
        setError('Failed to load theater data. Please try again.');
      } finally {
        setLoading(false);
      }
    };

    checkAdminAndLoadTheater();
  }, [user, router, theaterId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the theater using supabaseAdmin to bypass RLS
      ...
      setError(err.message || 'An error occurred while updating the theater.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold">Edit Theater</h1>
