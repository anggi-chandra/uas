...
export default function EditShowtimePage() {
  ...
  const [showtimeLoaded, setShowtimeLoaded] = useState(false);
  ...
  useEffect(() => {
    ...
    const checkAdminAndLoadData = async () => {
      try {
        ...
        // Load showtime data
        const { data: showtimeData, error: showtimeError } = await supabaseAdmin
          .from('showtimes')
          .select('*')
          .eq('id', showtimeId)
          .single();
        ...
        if (showtimeError) {
          if (showtimeError.code === 'PGRST116') {
            // Showtime not found
        ...
        if (showtimeData) {
          setMovieId(showtimeData.movie_id || '');
          setTheaterId(showtimeData.theater_id || '');
          setDate(showtimeData.date || '');
          setTime(showtimeData.time || '');
          setPrice(showtimeData.price?.toString() || '');
          setShowtimeLoaded(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We already have the booking count from when the component loaded
      // Use the stored booking count to determine what can be updated
      if (bookingCount > 0) {
        // If there are bookings, only allow updating the price
      ...
      setError(err.message || 'An error occurred while updating the showtime.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold">Edit Showtime</h1>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                <div>
                  ...
                  {bookingCount > 0 && (
                    <p className="mt-1 text-sm text-amber-600">
                      This showtime has bookings. Movie cannot be changed.
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {bookingCount > 0 && (
                    <p className="mt-1 text-sm text-amber-600">
                      This showtime has bookings. Theater cannot be changed.
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    {bookingCount > 0 && (
                      <p className="mt-1 text-sm text-amber-600">
                        This showtime has bookings. Date cannot be changed.
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {bookingCount > 0 && (
                      <p className="mt-1 text-sm text-amber-600">
                        This showtime has bookings. Time cannot be changed.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {bookingCount > 0 && (
                    <p className="mt-1 text-sm text-amber-600">
                      This showtime has bookings. Only the price can be updated.
                    </p>
                  )}
                </div>
